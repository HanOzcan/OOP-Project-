    ...
    public LoginGUI() {
        ...
        btn_login.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // hasta yı patient çevirme
                String secim = cmb_userType.getSelectedItem().toString();
                String tip = secim.equals("Hasta") ? "PATIENT" : "Doctor";

               
                Person user = UserFactory.createUser(tip, tc, "Test", "Kullanıcı", pass, "Genel");

                // Giriş Kontrolü
                if (user != null && user.login(tc, pass)) {
                    JOptionPane.showMessageDialog(null, "Giriş Başarılı! Hoşgeldiniz.");
                    
                    // Eğer giriş yapan bir Hasta ise Hasta Panelini aç
                    if (user instanceof Patient) {
                        PatientGUI pGUI = new PatientGUI((Patient) user);
                        pGUI.setVisible(true);
                        dispose(); // Giriş ekranını kapat
                    }
                } else {
                    // Veriler eşleşmezse veya Factory nesne üretemezse bu hata çıkar
